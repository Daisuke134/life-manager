...
- Repository hardening is now pushed on the dedicated branch: closed decision rows with replay keys and explicit quiet-hours fields are implemented and covered by the full Life Manager test suite. Production has not received this release yet; `LM_MENTAL_DECISION_LOG_REQUIRED=1` is intentionally held until the migration readback.
