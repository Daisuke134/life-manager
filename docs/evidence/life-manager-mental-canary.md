...
- The receipt row has `family=affirmation`, `window=morning_orientation`, and template `antara:courage-quiet:ja`. The provider-native Telegram history probe did not find its `telegram_message_id=1384`, so exact body/readback remains open and the structural evaluator's `telegram_content_readback=required` condition is not closed.
