...
**Live checkpoint (2026-09-18 03:18 UTC).** This supersedes the older counts below; the L-08
cursor and L-09 onward order stay in place. `contracts.json` is source-complete as of
`03:16:07 UTC`: incoming monthly offers 0, monthly contracts 0, received gross JPY 0.
The marketplace ledger has 201 `application_verified` receipts and no award, delivery or
...
|---|---|---|
| Apply | The installed main-origin SHA is `7e696a2f…`; project `5603444` was naturally reconciled to proposal `27942627` with one receipt, submit 0 and admission unknown 0. The ledger now has 201 verified applications. Older pending IDs `5601892` and `5601332` remain absent from complete proposal readback. | Continue natural discovery and measure proposal→reply→award. Never retry the older pending IDs blindly. |
| Browser attach | PR #5549 retries one Playwright CDP timeout even when `/json/list` is temporarily unavailable or has no stale auth target; first client is stopped and normal tabs are left intact. Apply and Work Sync at SHA `7e696a2f…` and Negotiate at a later main SHA `912dab05…` have natural passes. A live first-timeout→second-attach recovery under the new code has not yet been observed. | Observe any future attach timeout and exact attempt-2 outcome without restarting the browser owner or closing unrelated tabs. |
| Storefront | SHA `59770dd5…` has natural passes. Official `/myplan` showed 19 identical Web-app packages within the 20 active slot limit. Eighteen duplicates were individually moved to `非表示` with exact settings readback; canonical SNS listing `1338228` and one Web-app listing `1342394` remain. PR #5522 preserves the catalog cursor and serializes select→create→persist. Seven active official IDs exactly match canonical plus catalog state. The owner is currently fenced on an older uncertain run: a pre-submit catalog form failure and unchanged official listings were observed, but the same run also has Telegram provider ACK `87270`, which the available user-session could not independently read. | Keep the exact occurrence fenced until the report effect is conclusively reconciled; then apply the CDP retry release to Storefront. Track distinct listing inquiry/order counters rather than treating publication as conversion. |
| Negotiate / Work Sync | Source-complete inventory has 13 boards, including outreach board `9077726`, but required reply 0, incoming monthly offer 0, active monthly contract 0 and funded project 0. Work Sync had a 43-minute source gap under borrow/support admission. PR #5541 gives this exact observer revenue admission; its 90 pre-effect queued occurrences were reclassified under owner/control locks while queue position and one reservation were preserved. Main SHA `4a81d525…` passed natural wakes through the `02:35:44 UTC` complete snapshot. PR #5544 gives the single-worker shared Reply kernel an exact host pre-effect hint that is cleared before provider mutation or Telegram notification. Negotiate SHA `38332041…` passed a natural wake: 13 observed, effect 0, failed 0, admission unknown 0. One older thread `9064025` remains `reconcile_unknown` and requires its own official receipt check. | Keep the five-minute source current, reconcile that exact older thread, then act only on an exact buyer-last message or official offer/escrow record. |
| Paid | Historical queued occurrences repeatedly surfaced as `resource_effect_unknown` after pre-effect browser timeouts; each cleared claim had its own `pre_effect` marker. PR #5546 makes the shared four-worker Paid kernel retain an owned host hint before any worker mutation and clear it before a successful handoff to the Telegram reporter. Main SHA `df3d162e…` passed a natural Paid wake at `02:48:17 UTC`: active funded work 0, external effect 0, failed 0, admission unknown 0. The parallel timeout boundary passed focused tests and fresh read-only review but has not yet recurred live under this SHA. The provider adapter still raises `lancers_paid_effect_not_implemented` for actual mutation. | Keep exact occurrence recovery, then implement and prove funded delivery only when a real ContractReceipt exists; do not call a no-op wake proof of delivery. |
