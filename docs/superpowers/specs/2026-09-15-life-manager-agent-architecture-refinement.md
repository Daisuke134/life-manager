...
1. Repair Mobile Apps/Postiz once at the shared boundary, not account by account. Fresh readback proves
   ...
   A fresh 2026-09-24 02:06 JST capacity readback supersedes the earlier headroom observation. The Data
   volume has `346,226,688` bytes available, below the generic 512 MiB producer floor and the separate
   Writer high-water floor. The cleanup owner itself is alive and reaches terminal `pass` naturally on
   release `109f2b3966da3940a901014b4d6ce15d71d37560`; latest run
   `18d801e11b4187e8-95588` reports `errors=0`. Its business receipt nevertheless evaluates six host
   candidates, preserves five as `open`, reports 22 fast-inventory gaps, and leaves only
   `353,857,536` bytes after cleanup; its scratch pass evaluates and preserves all 532 entries. Thus the
   blocker is no longer described as a stopped cleanup job. It is a self-healing coverage/ownership gap:
   the running cleanup owner has no presently authorized large reclaim target. Do not infer that an open
   path is stale, delete unattributed data, or bypass admission. The next foundation atomic is to bind the
   preserved candidates and inventory gaps to an exact owner/liveness decision, reclaim only proven
   regenerable closed data, and prove stable free space above 512 MiB before the 08:30 Mobile canary.
...
| Shared host capacity | Earlier wakes defer as `resource_capacity_busy` and `resource_database_busy`. The former is shared resource-slot admission and the latter is the admission SQLite lock; neither is a direct synonym for disk-floor failure. Generic producer admission uses a 512 MiB floor. The 1,155,780,608-byte value is a separate Writer high-water floor. A fresh 2026-09-24 02:06 JST readback shows `346,226,688` bytes free. Cleanup naturally passes on `109f2b3966…`, but its latest receipt preserves five evaluated host candidates as `open`, reports 22 fast-inventory gaps, and leaves `353,857,536` bytes after cleanup. | Capacity is not currently sufficient for a truthful Mobile production canary or a broad local suite. This is not a stopped-cleanup defect: the cleanup control path runs, but its ownership/liveness evidence authorizes no large reclaim target. The post-fix broad test attempt therefore ends in pytest `tmp_path` setup errors, while direct affected tests still pass. Slot/DB contention, disk headroom and an individual loop failure remain separate boundaries. | Make preserved-candidate and inventory-gap ownership observable, reclaim only exact owner-proven/regenerable closed data, and prove stable space above the generic 512 MiB floor before the Mobile canary; Writer's higher floor remains separate. Preserve FIFO admission, and never guess-delete unattributed data or repeatedly kick Affiliate. |
...
The immediate ordered TODO is therefore: (1) close the cleanup coverage/ownership gap and prove stable host
space above the generic 512 MiB producer floor without guessing at unattributed data, (2) observe the
Mobile/Postiz 08:30 JST natural canary and exact provider readback, (3) prove its second same-SHA replay-zero and only then roll the
