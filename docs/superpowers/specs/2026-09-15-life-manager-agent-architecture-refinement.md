...
**Current Connector cursor (read-only state, 2026-09-18):** **REPLAY GATE OPEN.** The active native route is Luma/Connpass only, with the existing 1800-second interval; X is not a Connector provider or fallback. Connector-owned fixes are merged through PRs #5514, #5517, #5521, #5527, #5530, #5532, #5534, #5548 and #5559. The Connector label is now installed on complete main-derived release `/Users/anicca/loops/releases/20260918T231240-8eae8689`, SHA `8eae86892e391dc94325e9c1d0629d116c5edd03`; loaded/event SHA agree after the targeted reconcile. The latest manual kickstart occurrence `life-manager-connector-native:18d66feb19faf650-20958` was safely deferred before provider work with `host_admission_deferred:resource_control_busy` and remains `queued/effect_unknown=0`; do not retry while it is queued. The prior natural wake `wake-68e2dbebfedba65b4e6b4f1b` on the previous loaded SHA ended `completed_no_effect / providers_exhausted` after paid/restricted Connpass tiers and no Calendar-free Luma candidate. The focused Connector tests pass; CI's recorded failures remain unrelated `skills/_shared` inventory mismatch and main-owned registry fixture drift.
**Current Connector cursor (read-only state, 2026-09-18):** **REPLAY GATE OPEN.** The active native route is Luma/Connpass only, with the existing 1800-second interval; X is not a Connector provider or fallback. Connector-owned fixes are merged through PRs #5514, #5517, #5521, #5527, #5530, #5532, #5534, #5548 and #5559. The Connector label is installed on complete main-derived release `/Users/anicca/loops/releases/20260918T231240-8eae8689`, SHA `8eae86892e391dc94325e9c1d0629d116c5edd03`; the latest wake `wake-61848e3003edbc88d323e912` reached terminal `completed_no_effect/providers_exhausted` on that same SHA, with Submit 0, effect_unknown 0, and occurrence released. Connpass candidates were paid/restricted and Luma had no Calendar-free candidate. The preceding manual kickstart occurrence `life-manager-connector-native:18d66feb19faf650-20958` was safely cancelled after `resource_control_busy`; do not retry it. The focused Connector tests pass; CI's recorded failures remain unrelated `skills/_shared` inventory mismatch and main-owned registry fixture drift.
...
**Current done:** route restriction; model-driven ranking; bounded ranking/dispatch/action audits; official pre/post readback fencing; paid-only Connpass no-effect handling; preservation of `candidate_ref + error_class`; no browser fallback or circuit escalation for paid-only candidates after `f2b2550b`.

**Current remaining TODO, in order:**

1. Let the existing admission path recover queued occurrence `life-manager-connector-native:18d66feb19faf650-20958` from `resource_control_busy`; do not hand-kick it, stop siblings, or edit the DB. Require a natural terminal before claiming provider behavior.
2. Let the shared owner reconcile `session-vault:18d606ad782b0140-83191` through the existing path, then load release `8eae86892e` for `ai.anicca.session-vault` and verify Connpass/Luma keepalive readback. Do not edit `runtime/host`, `runtime/loop`, registry, or admission SQLite from the Connector worktree; a stale effect fence is a foundation handoff, not a provider Submit failure.
