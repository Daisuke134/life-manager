...
**Current Connector cursor (read-only state, 2026-09-18):** **NOT DONE.** The active native route is Luma/Connpass only, with the existing 1800-second interval. Connector-owned fixes are merged through PRs #5514, #5517, #5521, #5527, #5530 and #5532; the latest main SHA is `f2b2550b855554eeecec86fe515905d65f3b18a8`, and the idle Connector label is loaded from `/Users/anicca/loops/releases/20260918T101116-f2b2550b` with the same SHA. The focused Connector tests and `bin/lm-loop-contract` pass; CI's only recorded failure remains the unrelated `manifest_inventory_mismatch skills/_shared` check.
