...
- Normal Connpass authentication is still unresolved. The SSOT has no Connpass password. The existing X session reaches Connpass OAuth, but the consent page requests broad sensitive X permissions (private timeline, posting/deletion, follows, blocks, and profile changes); this was not approved because it exceeds the Connector need. The blank target was restored without submitting consent. Do not claim the official 404714 `registered` replay gate closed until a normal authenticated Connpass session is available.
- After the browser/release correction, targeted Connector apply succeeded (`c9fc8a65`) but the official 404714 readback remains `login_required`; no Connpass tab is authenticated on the Connector-owned 9222 profile. The remaining boundary is still account authentication, not provider discovery or shared admission.
