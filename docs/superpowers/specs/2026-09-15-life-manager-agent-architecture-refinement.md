...
3. Have the shared session-vault owner decide the keepalive coverage for authenticated Connpass/Luma URLs. Do not edit `runtime/host`, `runtime/loop`, registry, or the shared tick from the Connector worktree; a missing keepalive is a foundation handoff, not a provider Submit failure.
4. Merge/load PR #5608's owner-aware orphan-context census. Then add the proof-gated recovery path: close only stale contexts with an owner-backed lease row, leave unknown/user/other-owner contexts untouched, and record before/after target counts.
