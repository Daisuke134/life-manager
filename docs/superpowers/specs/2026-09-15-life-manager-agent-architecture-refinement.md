...
1. Review and integrate the Connector-owned browser-foundation patch `b5909abffd` through the normal main/release path; then confirm the loaded Connector release contains it. Do not call a branch test or a pushed SHA production evidence.
2. Have the shared session-vault owner decide the keepalive coverage for authenticated Connpass/Luma URLs. Do not edit `runtime/host`, `runtime/loop`, registry, or the shared tick from the Connector worktree; a missing keepalive is a foundation handoff, not a provider Submit failure.
...
4. Let the next natural wake run on the loaded dependency-complete release and confirm event/install SHA alignment, Luma/Connpass discovery, terminal state and browser lease cleanup.
