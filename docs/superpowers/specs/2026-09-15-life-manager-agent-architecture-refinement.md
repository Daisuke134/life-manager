...
**Session persistence boundary:** `skills/browser/scripts/session_vault_tick.sh` originally warmed Coconala, Instagram and X only; it did not keepalive an authenticated Connpass or Luma URL. PR #5615/main `8eae86892e` now adds the existing `https://connpass.com/dashboard/` and `https://luma.com/home` URLs to that same 30-minute tick, with focused vault tests passing. Complete release `/Users/anicca/loops/releases/20260918T231001-8eae8689` contains the change. The current `ai.anicca.session-vault` status has recovered to `pass`, but its installed/event SHA is still the old `0220d456`; the keepalive release has not been loaded and the historical `session-vault:18d606ad782b0140-83191` row remains visible. Do not edit admission SQLite or clear it manually. A live `session_vault.py status` readback showed 1,522 cookies with an age of 21.8 hours and no Connpass/Luma origin in its reported logged-in-origin list. This is evidence of a session-vault/keepalive coverage gap, not evidence of a malicious process or a Connector provider crash. Until an official Connpass page reads `registered`/`pending`, no 404714 resend is allowed.
...
**Current done:** route restriction; model-driven ranking; bounded ranking/dispatch/action audits; official pre/post readback fencing; paid-only Connpass no-effect handling; preservation of `candidate_ref + error_class`; no browser fallback or circuit escalation for paid-only candidates after `f2b2550b`; terminal no-effect reporting now preserves a single-provider `*_candidates_ineligible` boundary instead of collapsing it to `providers_exhausted`.
...
4. Restore an ordinary authenticated Connpass session in the Connector-owned 9222 profile and read the official 404714 page as `registered`/`pending` before any retry. Do not treat an X OAuth provider or broad social permissions as part of Connector.
...
3. **CN-C03 — Real Connpass acceptance:** **PASSED ONCE; REPLAY GATE OPEN.** New event `404714` has official `registered`, provider receipt `c0cfd9c2…`, Calendar ID `j48ajkcqtnenng15omi307ht34`, independent exact-one API readback, Telegram IDs `87476`/`87477` and durable bundle `a02d88cfe12f…`. The current remaining gate is PR #5559 merged into a loaded release, a verified Connector login session, then two same-SHA natural replay-zero wakes.
