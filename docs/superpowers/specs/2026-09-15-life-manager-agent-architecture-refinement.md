...
**Current Connector cursor (read-only state, 2026-09-18):** **REPLAY GATE OPEN.** The active native route is Luma/Connpass only, with the existing 1800-second interval; X is not a Connector provider or fallback. Connector-owned fixes are merged through PRs #5514, #5517, #5521, #5527, #5530, #5532, #5534, #5548 and #5559. The currently loaded dependency-complete release is `/Users/anicca/loops/releases/20260918T211134-c9fc8a65` with SHA `c9fc8a6585f5fbdc389417b982eb701744fa07a4`; status event SHA is stale until the next natural wake. The original effect run loaded `/Users/anicca/loops/releases/20260918T120510-919e3484` with SHA `919e3484202147094c33b5874399b00fba09dd2c`. Latest CLI wake `wake-65f078e868657b2f9d2af86a` ended `completed_no_effect / providers_exhausted`: Luma `free_open=2 / calendar_free=0`; Connpass `calendar_free=9` but provider attempts were paid/restricted tiers. The focused Connector tests pass; CI's recorded failures remain unrelated `skills/_shared` inventory mismatch and main-owned registry fixture drift.
...
1. Let the shared admission/release owner reconcile occurrence `life-manager-connector-native:18d66aef19152518-55543` (`claimed/effect_unknown=1`) through the existing path. Do not stop a live sibling, reclaim by timeout, clear the DB, or replay the provider action. Re-run targeted loaded-idle reconcile only after the fence is released/known.
2. Confirm the loaded Connector argv/install/event SHA is the complete main-derived `5b6e178835` release and read a natural terminal. A branch test, pushed SHA, or cut release alone is not production evidence.
3. Let the shared owner reconcile `session-vault:18d606ad782b0140-83191` through the existing path, then load release `8eae86892e` for `ai.anicca.session-vault` and verify Connpass/Luma keepalive readback. Do not edit `runtime/host`, `runtime/loop`, registry, or admission SQLite from the Connector worktree; a stale effect fence is a foundation handoff, not a provider Submit failure.
...
5. Restore an ordinary authenticated Connpass session in the Connector-owned 9222 profile and read the official 404714 page as `registered`/`pending` before any retry. Do not treat an X OAuth provider or broad social permissions as part of Connector.
