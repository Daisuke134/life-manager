...
1. Shared foundation owner: add the proof-bound `session-vault` no-effect recovery in `runtime/loop/lm_loop_run.py::_run_admitted` and its focused test, then reconcile `session-vault:18d606ad782b0140-83191`, load release `8eae86892e`, and verify Connpass/Luma keepalive readback. Do not edit `runtime/host`, `runtime/loop`, registry, or admission SQLite from the Connector worktree; a stale effect fence is a foundation handoff, not a provider Submit failure.
2. Add the proof-gated orphan recovery path on top of merged PR #5608: close only stale contexts with an owner-backed lease row, leave unknown/user/other-owner contexts untouched, and record before/after target counts.
3. Restore an ordinary authenticated Connpass session in the Connector-owned 9222 profile and read the official 404714 page as `registered`/`pending` before any retry. Do not treat an X OAuth provider or broad social permissions as part of Connector.
4. When a new free, open, Calendar-free Luma or Connpass event appears, require provider registration ID → Google Calendar event exact 1 → independent Calendar readback → bundle/Telegram receipt. If no such event appears, record truthful no-work and keep the 30-minute schedule.
5. After a new effect, observe two subsequent natural wakes on the same loaded SHA with official provider readback still `registered`/`pending`, Submit 0, Calendar exact count 1 and duplicate 0.
6. Only after items 1–5 pass, close Connector in this spec and advance to Coconala Apply, Reply, Paid and Storefront. Do not convert a deferred, login-required or no-work wake into a success claim.
