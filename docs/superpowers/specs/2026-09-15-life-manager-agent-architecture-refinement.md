...
- [ ] Check `gh pr checks 5321` after any branch update. Once all required jobs pass, make PR #5321 ready and merge it with `gh pr merge 5321 --admin --merge`. Record the merge SHA and verify `origin/main` contains the two changed Connector code paths. Do not merge the other A15 worktree or a broad candidate branch.
