...
Current cursor is step 3's A15d2/A15 fleet gate: the predecessor-SHA
release-reconciler is now loaded-idle after an outer `pass`; sync the already-built
`20c6067c3d` release to this one label, then observe a natural successful terminal and a bounded
...
| 3 | **Current cursor — prove the foundation in production:** main `20c6067c3d` and its immutable release exist. Finish A15d2: the old-SHA release-reconciler is now loaded-idle; sync this one control-plane label and observe a natural terminal. Finish A15: bounded queue-age/recovery evidence with no starvation; retain unknown-effect fences for effect-bearing lanes. | Exact loaded argv/SHA and automatic transitions exist for representative agent/browser/deterministic owners, but 100 queued rows and an oldest wait of about 666 minutes contradict a fleet Done claim. Do not advance on a pass from one owner. |
...
- [ ] A15d2 — The predecessor-SHA release-reconciler is loaded-idle after an outer `pass` at the latest read. Sync only that label to `20c6067c3d` and require a natural terminal without the managed-Node error. No restart or force-merge is part of this step.
