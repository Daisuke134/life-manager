...
| 対象 | 実測状態 | 判定 |
|---|---|---|
| source | `origin/main=437b5696d2463d6d0c929d2613782c088c0f9cc6`（PR #5249）; lease修正branch `fa14b77137` はpush済み・未統合 | Lancersの容量・planner・遷移診断・exhaustive 1-turn・pending cursor・本番wrapperのbounded discovery・timeout診断・single-slice discovery・Playwright cleanup watchdog・提案DOM読取・Paid cleanup・全marketplace収益admission・admission v2自動復旧・bounded reconciler・Lancers finite laneの5分timeoutを含む。branchにはshared browser session lease、解放回帰テスト、Lancers Skillの運用規則を追加 |
...
| 11 | `BROWSER-04` shared Lancers browser session leaseをmain由来releaseへ反映 | Lancers browser boundary + all Lancers owners | **branch `fa14b77137` code/tests/live contention/Paid canary PASS**: lease保持中の別processは`browser_session_busy`で拒否し、解放後の2本のread-only inventoryとbranch版Paid ownerは順番に成功。ページ終了時のlease解放も回帰テスト済み。main merge後、Application/Paid/Reply/Storefrontの2 natural wakeで競合0を確認 |
