...
**Current Connector cursor (read-only state, 2026-09-18):** **REPLAY GATE OPEN.** The active native route is Luma/Connpass only, with the existing 1800-second interval. Connector-owned fixes are merged through PRs #5514, #5517, #5521, #5527, #5530, #5532, #5534 and #5548; main has since advanced to `919e3484202147094c33b5874399b00fba09dd2c`, and the effect run loaded `/Users/anicca/loops/releases/20260918T120510-919e3484` with that SHA. The focused Connector tests and `bin/lm-loop-contract` pass; CI's only recorded failures are the unrelated `skills/_shared` inventory mismatch and main-owned registry fixture drift.
...
3. **CN-C03 — Real Connpass acceptance:** **PASSED ONCE; REPLAY GATE OPEN.** New event `404714` has official `registered`, provider receipt `c0cfd9c2…`, Calendar ID `j48ajkcqtnenng15omi307ht34`, independent exact-one API readback, Telegram IDs `87476`/`87477` and durable bundle `a02d88cfe12f…`. The current remaining gate is two same-SHA natural replay-zero wakes.
...
5. **CN-C05 — Repeatability:** **CURRENT GATE OPEN.** The historical `405297` replay-zero remains separate. New effect `404714` is now proven once on `919e3484`; two subsequent natural wakes must leave Submit at 0 and Calendar exact count at 1 on the same SHA. The configured interval remains 1800 seconds.
