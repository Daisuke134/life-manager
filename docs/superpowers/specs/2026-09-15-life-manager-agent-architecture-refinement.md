...
This plan implements step 4 above in the existing `life-manager-connector-native` owner. It changes no A15 file or state. The code owner uses `/Users/anicca/Projects/life-manager-connector-coconala-20260917`; the spec owner uses this file. Each checkbox is a separate evidence gate. A cheaper executor may follow the commands and acceptance text literally; it must stop an unsafe external action, not weaken a check to obtain a green result. **Dais resumed implementation. PRs #5321 and #5326 are merged and the main-derived `5a1117e6` release is now loaded. Do not repeat their merge, release apply, or the queued occurrence's kickstart. Current cursor: wait for that occurrence or a natural Connector wake to reach the provider, then resume CN-C03.**
...
**Connector questionnaire notification repair (branch, not live yet):** Commit `965b8c7` on `codex/connector-coconala-20260917` captures bounded public required-question labels from the existing Connpass DOM guard, carries them through the existing workflow, and sends one durable, idempotent `connpass-questionnaire-requests.jsonl` Telegram receipt through `connector-connpass-action-telegram.js` after the browser fallback also fails. It never fills a radio/checkbox/select, never invents consent or a profile fact, and never calls Submit from the notification path. The focused Connpass/provider/runner/production/browser-harness tests are 400/400 green after merging main `4835540a`; the branch is pushed but not merged or loaded. Next integrate this exact Connector diff into main, cut a complete release, apply only the idle Connector label, and run one natural canary. A notification receipt is not an external registration effect.
...
##### CN-C02: Integrate one Connector source and install one release (historical checklist; loaded)
