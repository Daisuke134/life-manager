...
**Current Connector cursor (read-only state, 2026-09-18):** **NOT DONE.** The active native route is Luma/Connpass only, with the existing 1800-second interval. Connector-owned fixes are merged through PRs #5514, #5517, #5521, #5527, #5530, #5532 and #5534; main has since advanced to `ab90915bdacbeac481353457e4d22ce55b7261b2`, and the Connector label is loaded from `/Users/anicca/loops/releases/20260918T105152-80af44be` with that main-derived release. The focused Connector tests and `bin/lm-loop-contract` pass; CI's only recorded failure remains the unrelated `manifest_inventory_mismatch skills/_shared` check.
...
1. Keep the loaded `f2b2550b` schedule active and let the next natural/approved wake reach a genuinely free/open Connpass event or a Luma event with `calendar_free>0`; the latest canary was truthful no-work because every ranked Connpass candidate was paid-only and Luma conflicted with Calendar.
2. For a selected event, prove official `registered`/`pending` and provider receipt, then independent Google Calendar exact-one event ID, matching canonical URL/private property, Telegram IDs and durable `applied_bundle`.
3. Observe two subsequent natural wakes on the same loaded SHA with repeat Submit 0 and Calendar duplicate 0; record target lease cleanup and terminal receipts.
4. Only after items 1–3 pass, replace the current row's open state and advance to Coconala's four independent lanes. If no eligible event is present, record truthful no-work and keep the Connector schedule active; never call no-work a registration.
...
3. **CN-C03 — Real Connpass acceptance:** **HISTORICAL BASELINE; CURRENT GATE OPEN.** Event `405297` has official `registered`, provider receipt, Calendar ID, independent exact-one API readback, Telegram IDs and durable applied bundle. That evidence remains valid but is not a new post-repair effect. The current gate requires a new free/open event.
...
5. **CN-C05 — Repeatability:** **HISTORICAL REPLAY-ZERO; CURRENT GATE OPEN.** Natural wakes `wake-953036ad8babec6ae03109f1` and `wake-96cc924ba6c56c1562a25308` used the same historical loaded SHA, left `405297` at repeat Submit 0, kept Calendar exact 1, continued to other candidates, and released browser ownership cleanly. The configured interval remains 1800 seconds; the current post-repair effect still needs two same-SHA natural replays.
