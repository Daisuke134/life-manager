...
**Current Connector cursor (read-only state, 2026-09-18):** **REPLAY GATE OPEN.** The active native route is Luma/Connpass only, with the existing 1800-second interval. Connector-owned fixes are merged through PRs #5514, #5517, #5521, #5527, #5530, #5532, #5534 and #5548; the new login-wall classification is in PR #5559 (`219273eac6`) and is not yet in a production release. Main has since advanced to `20eeb4eba665fcff6c72c109ac1b76dfda900500`; the effect run loaded `/Users/anicca/loops/releases/20260918T120510-919e3484` with SHA `919e3484202147094c33b5874399b00fba09dd2c`. The focused Connector tests pass; CI's recorded failures remain unrelated `skills/_shared` inventory mismatch and main-owned registry fixture drift.
...
5. **CN-C05 — Repeatability:** **CURRENT GATE OPEN.** The historical `405297` replay-zero remains separate. New effect `404714` is proven once on `919e3484`; after the login-wall fix is loaded and the owned session is authenticated, two subsequent natural wakes must leave Submit at 0 and Calendar exact count at 1, with official Connpass readback still `registered`. The configured interval remains 1800 seconds.
