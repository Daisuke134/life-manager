...
| 対象 | 実測状態 | 判定 |
|---|---|---|
| source | `origin/main=437b5696d2463d6d0c929d2613782c088c0f9cc6`（PR #5249）; lease修正branch `4ea15a0544` はpush済み・未統合 | Lancersの容量・planner・遷移診断・exhaustive 1-turn・pending cursor・本番wrapperのbounded discovery・timeout診断・single-slice discovery・Playwright cleanup watchdog・提案DOM読取・Paid cleanup・全marketplace収益admission・admission v2自動復旧・bounded reconciler・Lancers finite laneの5分timeoutを含む。branchにはshared browser session lease、解放回帰テスト、runtime停止完了までのlease保持、Lancers Skillの運用規則を追加 |
| release selector | `~/loops/current`は`20260916T041632-437b5696`（SHA `437b5696`）を指す。Lancers 7 ownerのinstalled/loaded argvは全てcurrentへ移行済み | currentは全tree (`release_paths=ALL`)でmainと一致。release drift gateはinstalled/loaded基準で解消。直近eventの一部はe789履歴のため、current自然wakeのterminal確認は継続。protocol v2は有効、全体gateは未完 |
| Lancers Application | current移行前のe789自然wakeでproposal 27922413/27921564/27920095/27920655/27920683/27921551/27918648/27920014/27921097/27923559/27920139/27933426/27921626/27919994/27922946/27919395、current自然wakeで27920107/27920025を`application_verified`。pendingは134→105、同じeffectの重複ledger行なし | **Application canaryの公式効果/readbackはPASS**。current自然wakeも`pass`。残りpending 105件と全lane収益gateは未完 |
...
| Lancers ledger | 最新はsequence 165（proposal 27920025）。148〜165は`application_verified`のみで、paid/delivery receiptは0 | Application 18件は成功として記録。収益・納品の成功は0件として扱う |
...
| 6 | `PROD-01-F` pending 105件を順番に再確認し、fresh sliceを枯らさない | Application state/reconciliation only | 観測cursorではpendingが134→105（29件減）。18件の公式verified ledgerを確認済み。transient failureは次wakeへ送り、state invalidはfail-closed。105件の解消または理由付きterminalが必要 |
...
| 8 | `PROD-01-H` Lancers全7 ownerのrelease driftを解消 | Lancers ownerだけ | **installed/loaded gate PASS**: 7 ownerのplist・loaded programがcurrent SHA `437b5696`で一致。直近eventのe789履歴はcurrent自然wake後に更新する |
...
| 12 | `PROD-02` Lancers Negotiate/Storefront/Paidを個別に閉じる | 各provider adapter | laneごとの公式receiptまたは明示的not-applicable、重複0 |
| 13 | `MARKET-02` CrowdWorksを同じshared kernelで検証 | CrowdWorks adapter/owner | 応募・契約の公式receiptまたはtruthful not-applicable |
| 14 | `MARKET-03` Mercorをhuman gate付きで検証 | Mercor adapter/owner | typed gate再開、公式application/contract/payment receipt |
| 15 | `MARKET-04` Coconalaを別workstream完了後に検証 | Coconala owner | 既存の4 laneごとの公式receipt、buyer readback、replay-zero |
| 16 | `CONTROL-01` registry外Browser provisionerの所有者分類とhandoff | control plane + provisioner owner | active profileを止めずにdoctorのunmanaged 0、owner/release/state/readbackを登録 |
| 17 | `LOCAL-01/02` 14 Product Loopのlocal completion manifest | shared control plane + 各owner | unknown 0、未対応は明示状態、外部成功はreceipt限定 |
| 18 | `CLOUD-01/02/03` tenant分離・cloud Browser・phone-only経路 | cloud adapter | localと同じcontract、cloud canary、静かな通知、公式readback |
| 19 | `CLOUD-04` production昇格 | primary release owner | local gate、cloud gate、公式効果、replay-zeroの全PASS |
...
| Lancers | 公開案件→proposal、購入前会話、menu掲載、契約/納品。`www.lancers.jp`のwork/proposal/menu/myplan route、専用9227 CDP、safety verifier | Coconalaと同じtyped lifecycle・admission・effect/readback契約 | mainはPR #5249（437b5696）まで統合済み。proposal 18件を公式ledger sequence 148〜165へ記録（重複0）。7 ownerのinstalled/loaded argvはcurrentへ移行済み。Application/Negotiate/Paidのcurrent自然wakeはpass、Telegram-report/Work-syncはFIFO保留。現行コードではreport/work-syncの2 client同時接続を観測、shared browser lease branch `4ea15a0544`は未統合。pending 105件、paid/delivery receipt 0。protocol v2はversion 2で有効 | lease修正をmain由来releaseへ反映→同時接続0を確認→pendingを1 sliceずつ処理→各laneの公式receipt/readback→replay-zero |
