...
4. Have the shared browser owner add an owner-aware orphan-context census/recovery path. It must report context ID, page URLs and owner/lease evidence, close only proven stale owned contexts, and leave unknown/user/other-owner contexts untouched. Add a focused failure test before implementation.
