...
| 5 | **Current cursor — Affiliate local business repair.** Automatic admission-policy reconciliation is merged and naturally proved: run `18d7b90c10c01398-70755` on release `2fab674b28…` acquired the deterministic slot, reached outer `pass`, and released claimed occurrence `18d5fc6eb6f882b0-97975` with `effect_unknown=0`. PR #5778 is merged as `fa4a81285d…`; its bounded repair trusts exact public CTA/POST-endpoint readback before a local checkout, returns `ALREADY_LIVE`/`changed=false` for an already-published Substack article while preserving its pending notification, and converts revenue CDP/auth/schema/runtime failures to redacted actionable classes. Affiliate tests are 190/190 PASS, fresh review is SHIP and all PR checks are green. The main-derived immutable release `20260923T042427-fa4a8128` is current, but Affiliate is still finishing a pre-existing wake on loaded release `2fab674b28…`, so the new source has not yet produced a production wake. Official business evidence remains cumulative provider clicks 8, transactions 0 and approved/paid 0. | Do not interrupt the in-flight old-release wake. Let the reconciler apply `fa4a81285d…` at the next loaded-idle boundary; then require a natural wake with CTA official readback, Substack replay-zero, typed revenue result, occurrence release and `effect_unknown=0`. After that, repair the next exact business blocker and obtain an attributable conversion/payment receipt. Source/CI/release PASS is not affiliate revenue. |
