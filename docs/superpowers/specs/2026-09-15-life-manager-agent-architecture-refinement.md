...
**Current Connector cursor (read-only state, 2026-09-18):** **REPLAY GATE OPEN.** The active native route is Luma/Connpass only, with the existing 1800-second interval. Connector-owned fixes are merged through PRs #5514, #5517, #5521, #5527, #5530, #5532, #5534, #5548 and #5559; the login-wall fix is loaded in `/Users/anicca/loops/releases/20260918T211134-c9fc8a65` with SHA `c9fc8a6585f5fbdc389417b982eb701744fa07a4`, including the required `playwright-core` and `jsqr` bundles. Main has advanced beyond `a3b8421424`; the original effect run loaded `/Users/anicca/loops/releases/20260918T120510-919e3484` with SHA `919e3484202147094c33b5874399b00fba09dd2c`. The focused Connector tests pass; CI's recorded failures remain unrelated `skills/_shared` inventory mismatch and main-owned registry fixture drift.
...
1. Observe two subsequent natural wakes on the same loaded `919e3484` SHA with repeat Submit 0 and Calendar duplicate 0; record target lease cleanup and terminal receipts.
2. Confirm the bundle remains provider `registered`, Calendar exact count 1 and no duplicate bundle after both replays.
3. Only after items 1–2 pass, replace the current row's open state and advance to Coconala's four independent lanes. If a replay is deferred by admission, preserve the occurrence and wait for its natural terminal; never convert a deferred/no-work wake into a success claim.
