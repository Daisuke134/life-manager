...
| 5 | **Current cursor — Affiliate local business repair.** The bounded source fixes and automatic admission-policy reconciliation are merged. Main commit `a2d0e2d1c2…` contains the active-reservation starvation guard; later main `f86bacceab…` includes it and is current as immutable release `20260923T035050-f86bacce`. Natural run `18d7b90c10c01398-70755` on preceding main release `2fab674b28…` acquired the deterministic slot, reached outer `pass`, and released claimed occurrence `18d5fc6eb6f882b0-97975` with `effect_unknown=0`, proving recovery beyond the former `resource_admission_unavailable` boundary. The same official output reports Substack distribution `LIVE`, exact impressions 1,047 and cumulative provider clicks 8, but publication `PUBLICATION_FAILED`, `revenue.capture` transient failure, transactions 0 and approved/paid 0. | Keep the successful admission repair; on the next natural wake prove replay-zero for the live distribution, repair the exact publication/revenue-capture/CTA-instrumentation failures, then obtain an attributable conversion/payment receipt. An outer pass proves lifecycle progress, not affiliate revenue. |
