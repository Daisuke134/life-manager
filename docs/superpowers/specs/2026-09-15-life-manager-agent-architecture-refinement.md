...
| 対象 | 実測状態 | 判定 |
|---|---|---|
| source | `origin/main=2ff93374f79ccb91c712b98692537e3ce61387d7`（PR #5250）; 統合候補branch `fix/lm-fundamental-runtime-20260916`（HEAD `84dc640ffd`）はpush済み・未統合 | mainにはdynamic CDP endpoint修正が追加済み。統合候補にはshared browser session lease、stale claim解放の冪等化、Lancers Skill更新を追加し、関連554 tests + 17 subtestsがPASS |
...
| Lancers Application | e789自然wakeでproposal 19件、current自然wakeで27922237を含む`application_verified`。pendingは134→102、同じeffectの重複ledger行なし | **Application canaryの公式効果/readbackはPASS**。current自然wakeも`pass`。残りpending 102件と全lane収益gateは未完 |
...
| 6 | `PROD-01-F` pending 102件を順番に再確認し、fresh sliceを枯らさない | Application state/reconciliation only | 観測cursorではpendingが134→102（32件減）。20件の公式verified ledgerを確認済み。transient failureは次wakeへ送り、state invalidはfail-closed。102件の解消または理由付きterminalが必要 |
...
| 12 | `ADMISSION-02` stale claim解放を冪等化し、再予約を壊さない | shared runtime admission | **統合候補 `84dc640ffd` code/tests PASS**: stale sweepがclaimを先に削除しても`release_and_reserve`が警告・例外を出さず、必要な予約を継続。152 runtime tests PASS。main統合後、自然wakeで`resource release deferred`が0になることを確認 |
