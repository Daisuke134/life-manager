...
| Lancers Application | current移行前のe789自然wakeでproposal 27922413/27921564/27920095/27920655/27920683/27921551/27918648/27920014/27921097/27923559/27920139/27933426/27921626/27919994/27922946を`application_verified`。pendingは134→108、同じeffectの重複ledger行なし。current移行後の新wake待ち | **Application canaryの公式効果/readbackはPASS**。残りpending 108件と全lane収益gateは未完 |
| Lancers Browser | `loaded-running`、9227 CDPはChrome 145 / Protocol 1.3、Browser ownerはcurrent | Browser ownerのinstalled/loaded argvはcurrent。9227 listen canaryはPASS。provider効果はlane別に未確認 |
| Lancers Negotiate/Storefront/work-sync/report | 6 ownerのinstalled/loaded argvはcurrentへ移行済み。直近eventには旧e789の`entrypoint_exit_1`、`resource_capacity_busy`、`resource_fifo_wait`が残る | current自然wakeを各ownerで一つずつ実行し、terminalと公式readbackを確認 |
| Lancers Paid | 旧e789の約26分runをcurrentへ再読込。旧runは`entrypoint_exit_143`で終端、install event `39c9a112e9eba1f3c5e08bda` はSHA `437b5696`。current runは`provider_inventory`失敗後に単独read-onlyで再確認 | timeout canaryはPASS（無限実行を停止）。単独read-onlyとbranch版Paid owner canaryはログイン済み・契約候補0・残高0円・finance complete、`attempted=0/delivered=0`で終端。shared browser leaseはbranch `4ea15a0544`で実装・テスト済みだが本番未反映。契約が存在する場合の正式納品は別の認可・契約仕様が必要。公式paid receipt 0 |
...
| 6 | `PROD-01-F` pending 108件を順番に再確認し、fresh sliceを枯らさない | Application state/reconciliation only | 観測cursorではpendingが134→108（26件減）。15件の公式verified ledgerを確認済み。transient failureは次wakeへ送り、state invalidはfail-closed。108件の解消または理由付きterminalが必要 |
...
| Lancers | 公開案件→proposal、購入前会話、menu掲載、契約/納品。`www.lancers.jp`のwork/proposal/menu/myplan route、専用9227 CDP、safety verifier | Coconalaと同じtyped lifecycle・admission・effect/readback契約 | mainはPR #5249（437b5696）まで統合済み。現行DOMのproposal読取fallbackをe789で実行し、proposal 15件を公式ledger sequence 148〜162へ記録（重複0）。7 ownerのinstalled/loaded argvはcurrentへ移行済み。Paidはtimeout canary後にprovider_inventory失敗、単独read-onlyとbranch canaryは契約候補0・残高0円・finance complete、pending 108件、paid/delivery receipt 0。protocol v2はversion 2で有効 | current自然wakeを一件ずつ確認→pendingを1 sliceずつ処理→各laneの公式receipt/readback→replay-zero |
