...
1. Restore an ordinary authenticated Connpass session in the Connector-owned 9222 profile. Do not treat an X OAuth provider or broad social permissions as part of Connector.
2. Let the next natural wake run on the loaded dependency-complete release and confirm event/install SHA alignment, Luma/Connpass discovery, terminal state and browser lease cleanup.
3. When a new free, open, Calendar-free Luma or Connpass event appears, require provider registration ID → Google Calendar event exact 1 → independent Calendar readback → bundle/Telegram receipt. If no such event appears, record truthful no-work and keep the 30-minute schedule.
4. After a new effect, observe two subsequent natural wakes on the same loaded SHA with official provider readback still `registered`/`pending`, Submit 0, Calendar exact count 1 and duplicate 0.
5. Only after items 1–4 pass, close Connector in this spec and advance to Coconala Apply, Reply, Paid and Storefront. Do not convert a deferred, login-required or no-work wake into a success claim.
