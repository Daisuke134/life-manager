...
| 対象 | 実測状態 | 判定 |
|---|---|---|
| source | `origin/main=2ff93374f79ccb91c712b98692537e3ce61387d7`（PR #5250）; lease修正branch `4ea15a0544` とadmission修正branch `f2048a826f` はpush済み・未統合 | mainにはdynamic CDP endpoint修正が追加済み。Lancersの容量・planner・遷移診断・exhaustive 1-turn・pending cursor・本番wrapperのbounded discovery・timeout診断・single-slice discovery・Playwright cleanup watchdog・提案DOM読取・Paid cleanup・全marketplace収益admission・admission v2自動復旧・bounded reconciler・Lancers finite laneの5分timeoutを含む。未統合branchにはshared browser session leaseとstale claim解放の冪等化を追加 |
| release selector | `~/loops/current`は`20260916T044820-2ff93374`（SHA `2ff93374`）を指す。Lancers 7 ownerのinstalled/loaded argvは`437b5696`のままで、currentとのrelease driftが再発 | currentは全tree (`release_paths=ALL`)でorigin/mainと一致。Lancers ownerは次のbounded reconcileで一件ずつ2ffへ揃える必要がある。protocol v2は有効、全体gateは未完 |
| Lancers Application | current移行前のe789自然wakeでproposal 27922413/27921564/27920095/27920655/27920683/27921551/27918648/27920014/27921097/27923559/27920139/27933426/27921626/27919994/27922946/27919395、current自然wakeで27920107/27920025/27923989を`application_verified`。pendingは134→104、同じeffectの重複ledger行なし | **Application canaryの公式効果/readbackはPASS**。current自然wakeも`pass`。残りpending 104件と全lane収益gateは未完 |
| Lancers Browser | `loaded-running`、9227 CDPはChrome 145 / Protocol 1.3、Browser ownerはcurrent | Browser ownerのinstalled/loaded argvはcurrent。9227 listen canaryはPASS。currentコード未反映のためTelegram-report/Work-syncが同時接続した観測を記録し、lease反映が必要 |
| Lancers Negotiate/Storefront/work-sync/report | 6 ownerのinstalled/loaded argvはcurrentへ移行済み。Negotiateのcurrent自然wakeは`pass`、Telegram-report/Work-syncは`resource_fifo_wait`で保留。9227にはreport/work-syncの2 Playwright clientが同時接続した | current自然wakeを各ownerで一つずつ実行し、terminalと公式readbackを確認。shared lease反映後に同時接続0を確認 |
| Lancers Paid | 旧e789の約26分runをcurrentへ再読込。旧runは`entrypoint_exit_143`で終端、install event `39c9a112e9eba1f3c5e08bda` はSHA `437b5696`。current runは`pass`（契約候補なし） | timeout canaryはPASS（無限実行を停止）。current Paid runは`effect=0`・`readback=0`・公式paid receipt 0。単独read-onlyとbranch版Paid owner canaryはログイン済み・契約候補0・残高0円・finance complete。shared browser leaseはbranch `4ea15a0544`で実装・テスト済みだが本番未反映。契約が存在する場合の正式納品は別の認可・契約仕様が必要 |
| Lancers ledger | 最新はsequence 166（proposal 27923989）。148〜166は`application_verified`のみで、paid/delivery receiptは0 | Application 19件は成功として記録。収益・納品の成功は0件として扱う |

最新のmain統合はPR #5250（commit `2ff93374f7`）です。PR #5233で本番の
...
| 6 | `PROD-01-F` pending 104件を順番に再確認し、fresh sliceを枯らさない | Application state/reconciliation only | 観測cursorではpendingが134→104（30件減）。19件の公式verified ledgerを確認済み。transient failureは次wakeへ送り、state invalidはfail-closed。104件の解消または理由付きterminalが必要 |
...
| 8 | `PROD-01-H` Lancers全7 ownerのrelease driftを解消 | Lancers ownerだけ | **旧installed/loaded gate PASS → drift再発**: 7 ownerは`437b5696`で一致していたが、currentが`2ff93374`へ進んだため、次のbounded reconcileで再移行が必要 |
...
| 11 | `BROWSER-04` shared Lancers browser session leaseをmain由来releaseへ反映 | Lancers browser boundary + all Lancers owners | **branch `4ea15a0544` code/tests/live contention/Paid canary PASS**: lease保持中の別processは`browser_session_busy`で拒否し、解放後の2本のread-only inventoryとbranch版Paid ownerは順番に成功。runtime停止完了まで保持する解放回帰テスト済み。main merge後、Application/Paid/Reply/Storefrontの2 natural wakeで競合0を確認 |
| 12 | `ADMISSION-02` stale claim解放を冪等化し、再予約を壊さない | shared runtime admission | **branch `f2048a826f` code/tests PASS**: stale sweepがclaimを先に削除しても`release_and_reserve`が警告・例外を出さず、必要な予約を継続。152 runtime tests PASS。main merge後、自然wakeで`resource release deferred`が0になることを確認 |
| 13 | `PROD-02` Lancers Negotiate/Storefront/Paidを個別に閉じる | 各provider adapter | laneごとの公式receiptまたは明示的not-applicable、重複0 |
| 14 | `MARKET-02` CrowdWorksを同じshared kernelで検証 | CrowdWorks adapter/owner | 応募・契約の公式receiptまたはtruthful not-applicable |
| 15 | `MARKET-03` Mercorをhuman gate付きで検証 | Mercor adapter/owner | typed gate再開、公式application/contract/payment receipt |
| 16 | `MARKET-04` Coconalaを別workstream完了後に検証 | Coconala owner | 既存の4 laneごとの公式receipt、buyer readback、replay-zero |
| 17 | `CONTROL-01` registry外Browser provisionerの所有者分類とhandoff | control plane + provisioner owner | active profileを止めずにdoctorのunmanaged 0、owner/release/state/readbackを登録 |
| 18 | `LOCAL-01/02` 14 Product Loopのlocal completion manifest | shared control plane + 各owner | unknown 0、未対応は明示状態、外部成功はreceipt限定 |
| 19 | `CLOUD-01/02/03` tenant分離・cloud Browser・phone-only経路 | cloud adapter | localと同じcontract、cloud canary、静かな通知、公式readback |
| 20 | `CLOUD-04` production昇格 | primary release owner | local gate、cloud gate、公式効果、replay-zeroの全PASS |
...
| Lancers | 公開案件→proposal、購入前会話、menu掲載、契約/納品。`www.lancers.jp`のwork/proposal/menu/myplan route、専用9227 CDP、safety verifier | Coconalaと同じtyped lifecycle・admission・effect/readback契約 | mainはPR #5250（2ff93374）まで統合済み。proposal 19件を公式ledger sequence 148〜166へ記録（重複0）。currentは`044820-2ff93374`だが7 ownerは`437b5696`のためdrift再発。Application/Negotiate/Paidの直近自然wakeはpass、Telegram-report/Work-syncは容量/FIFO保留。現行コードではreport/work-syncの2 client同時接続を観測、shared browser lease branch `4ea15a0544`は未統合。pending 104件、paid/delivery receipt 0。protocol v2はversion 2で有効 | admission/lease修正をmain由来releaseへ反映→7 ownerを2ffへ再移行→同時接続0を確認→pendingを1 sliceずつ処理→各laneの公式receipt/readback→replay-zero |
