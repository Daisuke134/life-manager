...
1. Connector owner: merge/load PR #5631 only after its CI boundary is resolved, then verify one no-effect wake reports the provider-specific reason. This does not create an external effect and cannot close the registration gate.
2. Shared foundation owner: add the proof-bound `session-vault` no-effect recovery in `runtime/loop/lm_loop_run.py::_run_admitted` and its focused test, then reconcile `session-vault:18d606ad782b0140-83191`, load release `8eae86892e`, and verify Connpass/Luma keepalive readback. Do not edit `runtime/host`, `runtime/loop`, registry, or admission SQLite from the Connector worktree; a stale effect fence is a foundation handoff, not a provider Submit failure.
3. Add the proof-gated orphan recovery path on top of merged PR #5608: close only stale contexts with an owner-backed lease row, leave unknown/user/other-owner contexts untouched, and record before/after target counts.
4. **DONE — authentication/readback:** Restore an ordinary authenticated Connpass session in the Connector-owned 9222 profile using the existing GitHub login path, dump the session vault, and read official 404714 as `registered`. This is an existing registration, not the new-effect gate. X OAuth was not used.
5. When a new free, open, Calendar-free Luma or Connpass event appears, require provider registration ID → Google Calendar event exact 1 → independent Calendar readback → bundle/Telegram receipt. If no such event appears, record truthful no-work and keep the 30-minute schedule.
6. After a new effect, observe two subsequent natural wakes on the same loaded SHA with official provider readback still `registered`/`pending`, Submit 0, Calendar exact count 1 and duplicate 0.
7. Only after items 1–6 pass, close Connector in this spec and advance to Coconala Apply, Reply, Paid and Storefront. Do not convert a deferred, login-required or no-work wake into a success claim.
