...
| 6 | **Current cursor — close the capacity self-heal gap, then accept Mobile Apps/Postiz.** Host space recovers from below both floors to `2,976,153,600` bytes after two exact stale Sparkle updater helpers are terminated gracefully and their closed staging generations self-clear. Consecutive natural cleanup wakes remain `pass` on `109f2b3966…`, so the immediate outage is relieved. The permanent source candidate `d27e15a8a2…` is not accepted or loaded: fresh review finds a PID-reuse window, fail-open malformed-process/path cases and inaccurate termination/error receipts. Affiliate source repair remains complete and does not retain the portfolio cursor. | First make the cleanup candidate fail closed with an immediate full process-identity recheck, exact real-directory/inode checks, truthful signal/exit/error receipts and focused regressions; then merge and prove a natural main-derived cleanup wake without touching apps, browsers, Paid sessions or unattributed data. Next preserve Mobile's exact pending occurrence and official Postiz receipts, run one natural loaded-SHA canary plus same-SHA replay-zero, roll the shared contract across the remaining owners, repair Obou, and verify each enabled lane's three-per-day cadence from official receipts. |
...
1. Repair Mobile Apps/Postiz once at the shared boundary, not account by account. Fresh readback proves
   ...
   The capacity readback now supersedes the earlier low-headroom observation. The latest accepted natural
   cleanup receipt has `1,778,094,080` bytes available after its sweep, above both the generic 512 MiB
   producer floor and the separate `1,155,780,608`-byte Writer high-water floor. The measured root cause is
   two sleeping, same-user,
   ...
   `PersistentDownloads` staging trees have no open handles. After a same-operation identity readback,
   only those two fixed PIDs receive graceful `SIGTERM`; no app, browser, loop or Paid session is stopped,
   and the staging generations self-clear. Natural cleanup runs `18d8030486615f48-7649`,
   ...
   The operational recovery is now converted into an accepted production self-heal. Branch
   `fix/disk-cleanup-stale-sparkle-updater-20260924` at
   ...
   Important or Minor finding. PR #5816 passes every server check and merges to main as
   `1ac87e32ac26cc60b2e2b52cc2ee72ab57adb48f`. The natural release reconciler cuts complete immutable release
   `20260924T040401-1ac87e32`; targeted loaded-idle reconciliation changes only
   `life-manager-disk-cleanup`, with install event `f09820a45ea80aba9e163986` and exact loaded arguments rooted
   in that release. Its launch-on-load run passes, then the next unforced 300-second wake reaches a second
   terminal `pass` at `2026-09-23T19:12:38.530592Z` on the same SHA. The corresponding receipt has
   `errors=0`, `protected_deletions=0`, Sparkle recovery errors 0 and `free_after=1,778,094,080`; five open
   candidates are preserved and no updater is signaled because none is stale. The permanent cleanup source
...
2. In parallel through its natural owner schedule, complete Affiliate production acceptance without a
   ...
   `host-admission.json` with `effect=0`, `entrypoint-result.json` with
   `status=pre_effect_failure/effect=0`, and `.terminal-unrecorded`, but no report event. The immediate next
   owner report `1342bb1966b13080cba42411` is the `resource_effect_unknown` deferral at
   `2026-09-23T11:30:53Z`. The bounded window contains zero Affiliate external job events. A separate X job
   enters `EFFECT_STARTED/NOT_FOUND` about eleven hours later and therefore cannot prove or clear this older
   host occurrence.
   ...
   retained unknown therefore prevents the code capable of reconciling it from executing. The local candidate
   on `fix/affiliate-pre-effect-occurrence-recovery-20260923` moves Affiliate to occurrence-scoped admission,
   validates a private non-symlinked exact marker tied to owner/occurrence/runtime run, supports the unique
   legacy marker only when the single unknown row, dead scratch owner, execute/no-report window, immediate
   deferral and zero external events all agree, persists a secret-free evidence receipt, and calls the shared
   exact-occurrence resolver. It holds the current wake before business/provider work if proof is absent or
