...
- `scripts/verify-agent-contract.sh` now runs in the security workflow and declared security tests. It validates only the repository contract (canonical remote, root instruction import, Astra Advisor/Ponytail/Superpowers wording, cleanup contract, and no stale Felix/`anicca-project` references); the job passed in PR #5478 at `1c38b59eb496b626fd3fb284ff1ffd0984c3e5a5`. The local Claude `ssot-guard.sh` was also corrected to use `$HOME/Projects/life-manager-main` and to warn when its Git root is outside that checkout. These are drift guardrails, not proof of model reasoning or actual skill invocation.
- The nested instruction consolidation then closed the remaining duplicate loader paths. `control-room/CLAUDE.md` inherits `../AGENTS.md`; `skills/video/hyperframes/capafy-o13-review/CLAUDE.md` is exactly `@AGENTS.md`, and its local `AGENTS.md` retains the former body with matching SHA-256 (`a2cdd7633ef59be379dad6c2cc135bd8468bd64794423b6d3041bcb49f691b4a`). The agent-contract gate now requires the control-room loader plus both HyperFrames files and rejects missing files or extra HyperFrames instructions. Ten fixture tests, security-contract tests, OSS self-contained verification, shell syntax, and a fresh read-only review passed; PR #5483 merged at `67f5a92c5f530d556697f6318d4de02b9de9f252`. The guard still detects drift only; it cannot prove model behavior.
