...
- [x] Require the model to save a fresh local Profile/Résumé readback JSON beneath the current evidence directory and return that path; a Mercor URL alone cannot activate the profile-sync receipt.
- [x] Restore the successful discovery path from the 2026-09-16 run: query the visible Mercor Filter/Search surface with `Japan`, `Japanese`, `Developer`, `automation`, `AI agent`, and `Coding`, deduplicate the union, and defer opening existing incomplete cards until that queue is scanned.
