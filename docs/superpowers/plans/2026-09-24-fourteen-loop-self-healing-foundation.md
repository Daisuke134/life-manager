...
- [x] Diagnose `job-search-learning` read-only: the calendar-scheduled application-effect owner has one claimed `effect_unknown` occurrence, two queued occurrences and eight released occurrences, but no durable `priorities` row. Historical occurrences carry `deterministic/borrow/support`, yet there is no authoritative current policy source; the owner is loaded-idle and blocked at `resource_effect_unknown`. Do not invent a contract or retry an application. Record the policy-provenance gap and preserve the application fence until an accepted owner policy and official proposal/application readback exist.
...
- [ ] Resolve `job-search-learning`'s missing durable admission policy from an accepted source (not occurrence inference), then obtain official readback for its claimed application effect. Only after both are bound may a contract be added and the owner promoted/reconciled; until then keep it fenced and do not submit or replay an application.
