...
- [x] Close the Claude brain infrastructure gap on this branch. When `ANICCA_BRAIN=claude-p` cannot spawn the configured executable, `think()` now falls back to the configured proxy; OAuth, timeout and non-zero Claude exits remain typed failures and never silently switch brains. The direct brain contract is now aligned with the implementation (5/5), the missing-binary integration case passes, and the remaining integration failure is the pre-existing temporary-directory cleanup race.
