...
- [x] Close the Claude brain infrastructure gap on this branch. When `ANICCA_BRAIN=claude-p` cannot spawn the configured executable, `think()` now falls back to the configured proxy; OAuth, timeout and non-zero Claude exits remain typed failures and never silently switch brains. The missing-binary integration case passes; the remaining integration failure is the pre-existing temporary-directory cleanup race.
