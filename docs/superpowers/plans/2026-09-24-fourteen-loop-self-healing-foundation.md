...
- [x] Perform the 2026-09-25 JST eligibility audit before triggering that canary. The loaded recovery supervisor is exact-current `2f809c8621d9c6d92d96c68c01e2c99da8aea3eb`, but the durable queue has no non-self intent on that release that this workstream may safely execute: the current Connector intent is separately owned, while Agent Economy and the remaining candidates point to older releases. Read-only supervisor evidence records `healthy_readback_pending` for Connector and `release_sha_mismatch` for Agent Economy; no provider/browser/wallet effect was run. Keep the canary open until an eligible exact-release no-effect intent exists; do not forge one or wait on revenue.
...
- [x] Classify a `reconcile` result that is `skipped_pending` for the requested owner as a typed admission wait instead of a repair failure. `recovery-executor` now returns `queued / admission_pending / retry_after_eligibility` with `budget_consumed=false`; it preserves the exact before/after readback and reconcile payload. The RED regression reproduced `reconcile_target_not_applied`; the focused recovery suite is 33/33 after GREEN. This prevents a durable FIFO backlog from consuming the bounded repair budget or escalating as a broken owner. No production/provider state changed.
