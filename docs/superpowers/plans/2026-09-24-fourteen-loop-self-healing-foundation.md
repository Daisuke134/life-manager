...
- [ ] Align the final `affiliate-loop` base owner after its currently active old-release child reaches a terminal; do not stop it mid-effect. Then run the Affiliate foundation projection twice with exact current-release evidence, zero opaque diagnostics and replay-zero. Investment read-only diagnosis can overlap; revenue and a future natural scheduler wake are not gates.
