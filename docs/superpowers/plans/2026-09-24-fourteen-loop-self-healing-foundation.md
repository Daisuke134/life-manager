...
- [x] Add only Writer's observed admission/coalescing contract to all seven owners. The RED test fails for every missing row, focused contract/fixture tests pass 2/2 and full apply/registry passes 208/208. Do not change Writer business code or clear response/report during this source slice.
- [ ] Merge and release the Writer contract. Reconcile loaded-idle effect-free owners first, add the minimum archive-aware exact proof for response without weakening the current proof contract, and reconcile report only after exact Telegram provider readback; then align all seven owners and require exact-release complete diagnostics plus replay-zero.
