...
**Connector domain-retention audit:** `connector-minimal-production.js::discoverCandidates` merges fresh provider discovery with `reconciliation-candidates.json`, but that private store saves only a Connpass completed action or an `effect_unknown` candidate. Ordinary newly discovered candidates are not persisted before ranking/action; `connector-action-cache.json` stores verified browser control sequences, not business candidates. Therefore a host wake coalesced during a reservation is **not yet proven safe for Connector**: a candidate observed only during the omitted wake could disappear from later provider search. Next atomic domain work is a retained, owner-scoped candidate inbox keyed by provider/event_ref, with a real test that discovers two distinct candidates across overlapping wake signals, resumes each from the durable inbox, and removes only after official effect/readback or explicit no-action terminal. Preserve the existing uncertainty store and effect fences; do not relabel tentative discovery as already registered. Only then opt Connector into host wake coalescing. This is source analysis, not proof of any current provider effect.
