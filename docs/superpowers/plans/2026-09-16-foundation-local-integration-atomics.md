...
**Live nested-report separation:** At 2026-09-16 ~14:43 UTC, the installed main status reader displayed Coconala Paid `pass` while its loaded label and outer run `18d5d336ca66b6c0-26709` were still running. The later pass event belonged to `provider=codex`, profile `acct1`, run ID `cb392b5dfb5bdfc22b213b07`, not the outer `lm-loop-run` occurrence. Read-only `cf655388` candidate status on the same event file correctly returned `last_terminal_result=running`, PID `26709`. This is an actual Local observability readback for the source fix, **not** an outer terminal, Coconala effect, or permission to interrupt the Paid owner. Keep following that exact PID/run to terminal and official room readback.

**Paid same-run terminal:** That outer run ended `fail/entrypoint_exit_1` at 2026-09-16 14:52:24 UTC. Its latest private Paid receipt reports `status=failed`, `failed_step=remote_verifier`, `effect=0`, `readback=3`, `actionable=1`; the inner Codex passes at 14:37 and 14:52 were not outer completion. This separates an active domain verifier blocker from the earlier pre-effect disk-headroom failures. No new client send is proved; the Coconala owner must inspect the exact verifier evidence before retry, while foundation does not change provider-specific work.
...
**Bounded Local candidate release/apply:** Full immutable release `/Users/anicca/loops/releases/20260916T233946-cf655388` was cut from pushed candidate SHA `cf65538879532a461a164fc2705b95853235ba14` with `LOOPS_ACTIVATE_CURRENT=0`; its manifest says `release_paths=ALL`, runtime Python cache tag `cpython-314`, and the shared `current` remained main release `1519611c`. Candidate doctor reported 164 registry entries, missing/unmanaged zero, `ok=false` solely because the old Obou Mobile label is still installed despite its retirement classification. After UID/Directory Services/Aqua/GUI preflight PASS and loaded-idle checks, **only** Connector and then Instagram Metrics labels were target-applied to this candidate release, with install receipts `ae79f0c51c9a3fd894f72ae6` and `f6aa44993e408b114c0d7102`. Connector's loaded argv is exact candidate SHA; Instagram Metrics' plist now pins absolute Node/Python plus private `marketing.env`. TikTok Metrics, Paid, Mobile publishers and `current` remained on their prior releases. Both new-SHA natural wakes, Connector official provider/Calendar effects and Metrics Postiz readback are still open. Do not call this Local gate PASS from loaded argv alone.

**Obou retirement Local readback:** Exact destination SSOT states Instagram `@obou.anicca` belongs to ebook scope and is a Mobile `0/day` hold. Candidate registry marks its old Mobile label retired. With the label loaded-idle and GUI preflight PASS, `lm-loop apply` targeted **only** `ai.anicca.life-manager-anicca-obou-instagram`: `was_loaded=true`, `removed_plist=true`, `retired=true`. Follow-up launchctl readback found no service, the exact plist is absent, and candidate doctor is now `ok=true` with 164 registry entries, missing 0, unmanaged 0, retired-installed 0. No account, asset, receipt or ledger was deleted; the old plist can be regenerated from its prior immutable release if rollback is required. Connector, Paid and shared `current` retained their loaded release SHAs. This closes only the obsolete Mobile label drift, not ebook marketing or 17 active Mobile publications.
...
`8768edf168` then added validated exact-occurrence reads of the write-ahead
journal; 780 Connector, 131 host/runner and OSS checks remain green. A
resolver must still use each stored public candidate snapshot and effect
