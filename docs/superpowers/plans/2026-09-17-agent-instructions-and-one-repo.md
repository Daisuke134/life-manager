...
- [x] B3. Add deterministic source-boundary checks at the supported edit/push boundary: Git root and origin must match Life Manager. `scripts/verify-source-boundary.sh` and `npm run verify:source-boundary` now give clear failures; the canonical worktree passes and `anicca-project` is rejected. Agents must invoke this check before editing or pushing; no second hook framework was added.
