...
- [ ] C3. For each required missing asset, use a dedicated worktree and a small migration PR: copy source, update imports/build/deployment references, run focused verification, merge, and confirm the canonical path is used. Repeat only for verified required assets.
  ...
  - [ ] C3b. Audit the private Honne source at pinned revision `b57928bb13ef1f9a1e774e4bca2467e3059c9eac` and migrate only after private Git access, secrets, signing material and required source files are read back safely. Until then retain the descriptor and do not copy the private checkout.
