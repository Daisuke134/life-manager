...
- [x] B3. Add deterministic source-boundary checks at the agent edit/push boundary: Git root, Git common directory and origin must match Life Manager. `scripts/verify-source-boundary.sh` and `npm run verify:source-boundary` now give clear failures; the canonical worktree passes, `anicca-project` is rejected, and an independent clone nested under `.worktrees` is rejected by its different Git common directory. The check is an agent-invoked command rather than an automatic hook; no second hook framework was added.
