...
- [ ] D3. Report retained worktrees with owner and reason. Confirm the normal development checkout and current production releases still point to the intended source/release after cleanup.
  ...
  - [ ] D3b. The normal checkout is still dirty on `capafy/account-plan-deck-offline-20260912`. `~/loops/current` still points to immutable release `d29621be5159` with `provenance=ancestor-of-origin-main`; latest `origin/main` is now `a77c5629e1`. Read-only launchd inspection found the 17 loaded mobile labels (the OBOU label remains intentionally held out) using main-derived immutable releases `37384185…` for Anicca and most routes, `203bbe88…` for Honne EN, and `61036e1e…` for Honne JA. The `37384185…` tree contains both canonical mobile paths and the Postiz receipt suite is green, so no immediate release cutover is required for the migrated mobile source. A controlled rollout to a newer main-derived release remains open; do not mutate loaded jobs during an active posting window.
