...
test("native pass keeps only Luma and Connpass across every 30-minute slot", async () => {
  const observed = [];
  ...
  assert.deepEqual(observed.filter((_, slot) => slot % 2 === 0),
    Array.from({ length: 5 }, () => ["luma", "connpass"]));
  assert.deepEqual(observed.filter((_, slot) => slot % 2 === 1),
    Array.from({ length: 5 }, () => ["connpass", "luma"]));
  for (const providers of observed) assert.deepEqual(new Set(providers), new Set(["luma", "connpass"]));
});
