...
PROMPT="$PROMPT OWNER SCOPE: Dais deferred all IG marketing, Reels, HyperFrames, landing, and attribution rotation. This supply pass only develops and ships Capafy Agents, reads review status, and verifies key/revenue. Do not perform marketing actions."
PROMPT="$PROMPT DRAFT RESUME: when official inventory action is resume_draft, do not call publish_prepare or create another Agent version. Read the exact existing Agent work manifest and publisher-inputs contract; if absent or mismatched, fail closed with platform write 0. Continue CP1 using the official refreshed URL, then call publish_finish with that manifest's exact agent_version_id. Fresh/rejected version creation still uses publish_prepare only when review capacity is free."
UPDATE_ACTION="$(printf '%s\n' "$INVENTORY" | tail -n 1 | python3 -c 'import json,sys
d=json.load(sys.stdin)
if d.get("action") == "update_existing":
 r=d["item"]["update_request"]
 print(json.dumps({"agent_id":d["item"]["agent_id"],"from_version_id":r["from_version_id"],"target_model_id":r["target_model_id"]}))')" || exit 1
