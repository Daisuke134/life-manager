...
★ JUDGE BROKER — HARD BOUNDARY. ★ Every judge/vision model call is served by the wrapper-side judge broker through the model runner. When invoking any gate or the model runner, never clear, unset, or override ARTICLE_NESTED_SANDBOX, ARTICLE_RUN_DIR, or ARTICLE_JUDGE_BROKER_SERVER, and never bypass the judge broker by spawning a provider CLI directly: a direct provider spawn inside the bounded sandbox always fails, poisons provider health for the whole run, and forces every later safety gate to fail closed. Do not call a gate with --help; usage output is not a gate verdict. Invoke the exact command shape documented in this prompt, and if a usage error occurs, correct the arguments and rerun the gate rather than recording a safety failure. If a judge call returns no verdict, record the failure and leave the pair pending; do not retry with altered environment variables.
