...
mkdir -p "$TMP/skill/scripts" "$TMP/skill/../_shared/lib" \
  "$TMP/missing-repo/apps/life-manager/scripts/lib" "$TMP/state" \
  "$TMP/run/gates" "$TMP/home" "$TMP/bin"
...
SH
cat >"$TMP/skill/../_shared/lib/account-history.sh" <<'SH'
ah_record() { :; }
SH
cat >"$TMP/missing-repo/apps/life-manager/scripts/lib/load-env-file.sh" <<'SH'
lm_load_env_file() { :; }
SH
cat >"$TMP/bin/curl" <<'SH'
#!/usr/bin/env bash
...
HOME="$TMP/home" PATH="$TMP/bin:$PATH" \
  LIFE_MANAGER_REPO="$TMP/missing-repo" LIFE_MANAGER_ENV_FILE="$TMP/home/missing.env" \
  NOTE_URLNAME="test-writer" ARTICLE_SKILL_DIR="$TMP/skill" \
  ARTICLE_STATE_DIR="$TMP/state" ARTICLE_RUN_DIR="$TMP/run" ARTICLE_QUALITY_ADVISORY=1 \
...
echo "PASS: run.sh falls back to the immutable release shared library"
