...
def _repair_prompt(root: Path, item: Path, feedback: str, requirements_sha256: str,
    ...
    verifier_contract = (
        ...
        "The copied outcome must also preserve buyer_message_receipts from the builder. Verify one row for every buyer message after the "
        "latest seller reply, in official order. Each row must contain exactly message_id, requirement, resolution, and evidence_refs; "
        "evidence_refs must name builder official_receipts effect_key or readback_source values that independently prove that message was resolved. "
    ...
    ownership = (
        ...
        "remaining_work, official_receipts, and buyer_message_receipts. For every buyer message after the latest seller reply, in official order, "
        "write exactly one buyer_message_receipts row containing message_id, the model-decided requirement, its resolution, and nonempty "
        "evidence_refs naming official_receipts effect_key or readback_source values. Never mark the aggregate complete while any message lacks proof. "
