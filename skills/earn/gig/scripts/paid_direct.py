...
def _decision_prompt(context: Path, context_sha256: str, feedback: str,
    ...
    return (
        ...
        "Canonical example: a contract requires 300 sends, a seller message says 300 are complete, but the paired official "
        "provider ledger proves only 12; classify it actionable remote and continue from 12. A seller-authored total is never "
        "completion proof when official effect receipts contradict it. "
        "For every initial file submission and every correction, normally set delivery_stage to review and "
        "formal_approval_evidence to null. Formal delivery is never inferred merely because an artifact is complete. "
        "Set delivery_stage to formal after the buyer explicitly approves an already submitted artifact and permits "
        "the transaction to be completed. Also set it to formal when the latest buyer message explicitly ends the "
        "transaction and promises payment, the verified completed artifact is ready, and formal delivery is the seller's "
...
def _run_isolated_file_owner(args, root: Path, context: Path, prompt_text: str,
    ...
    with _project_workspace(
        ...
        prompt.write_text(
            ...
            + "Only if the buyer contract explicitly requires a native desktop application's editable output or proof "
            "of its use, and that required native desktop application cannot be controlled from this isolated process, "
            "do not fake its output and do not ask the buyer or operator "
            "to run it. Write delivery/paid-tool-requests.json with version=1 and a requests array. Each request "
            "must contain capability, input, output and receipt fields using paths relative to this workdir. Never request "
            "a native roundtrip merely because the controller supports it, a prior artifact used it, or a generic skill "
            "exists. Return "
...
def _build_and_authorize_file(args, item_path: Path, root: Path, item: dict[str, Any],
    ...
    owner_instructions = (
        ...
        "research is lengthy, or because a policy says the foreground supervisor must not do the loop's work. "
        "When a complete public verification can exceed one command's runtime, split it into deterministic bounded "
        "batches, persist row-level checkpoints and progress inside this staging root, and resume until every required "
        "row is terminal. A command timeout or interrupted monolithic fetch is execution feedback, not evidence that "
        "the buyer requirement is impossible and not permission to package the same incomplete limitation. Reserve "
        ...
        "only the named locator while leaving the same defect elsewhere. When the reviewer proves that the current "
        "candidate or source lineage cannot satisfy required coverage or provenance, do not repackage its limitations: "
        "resume public research and choose a different candidate or source lineage that can satisfy the contract. "
        "Preserving an accepted visual lineage never requires preserving a rejected subject, dataset, or claim. "
        "Canonical example: a reviewer proves that a performance sample lacks independently readable pre-event "
        "predictions. Renaming that sample vNext, recalculating it, or adding a clearer disclaimer is the forbidden "
        "next action because none supplies the missing provenance. The next action is to search public web and repository "
        "sources for a different sample whose pre-event prediction and official result can both be retained and replayed; "
        "only that evidence-backed replacement may become the next artifact. "
...
def _repair_prompt(root: Path, item: Path, feedback: str, requirements_sha256: str,
    ...
    ownership = (
        ...
        "verification_pending=true, both satisfied fields=true, and remaining_work=[]. The flags attest to the completed "
        "buyer effect and output, while the controller still withholds marketplace submission until verifier approval. "
    ...
    return (f"You are the {role}. PROJECT_ROOT={root}. Current queue item={item}. "
            ...
            f"Before claiming any buyer-provided value is missing, directly read {root / 'source/talkroom/messages.jsonl'} "
            "for every source_message_identities entry in required_outcomes. Redaction in compiled context is not evidence that the buyer omitted the value. "
            "Use values from those authenticated official rows only for the authorized effect, without copying private values into evidence or the customer message. "
            "Never ask the buyer to resend a value present in those official rows. "
            f"{target_contract} "
            ...
            f"{code_root / 'skills/browser/with-browser.sh'} with the resolver-selected TikTok identity; you must not create or invoke a "
            ...
            "profile and exclude prior effect keys before using the message transport. A profile with no posted content is invalid; "
            "the owner must not send or count it, even when a DM route, followers, likes, or an adult-looking profile name exists. "
            "Persist the official no-content readback as invalid screening evidence and continue with another candidate. If a prior "
            "send was counted before an official no-content readback was noticed, preserve its receipt and reclassify the same effect_key as invalid "
            "with classification_revision=true before calculating the effective total. Continue with another query when one "
            ...
            "Count cumulative outreach by unique eligible recipient, not by receipt or effect row: one eligible recipient handle can increment "
            "the cumulative total at most once. The TikTok DM exact readback and its matching Google Sheets row together prove one recipient pair; "
            "the Google Sheets receipt is supporting bookkeeping evidence and must use counts_toward_50=false. Reduce classification revisions by "
            "effect_key, then deduplicate counting TikTok DM rows by normalized candidate_handle; never count receipt rows or effect keys as recipient totals. "
            "Before reporting a total, reclassify any contradicted or ineligible TikTok DM effect and calculate verified_unique_sends only from the "
            "latest effective qualified TikTok DM row for each unique recipient with its matching Sheets readback. "
            ...
            "For every official provider readback, make the browser or API command atomically write its complete structured output to a "
            "project-owned file, then read and validate that file before deciding; stdout or truncated tool transport alone is never the "
            "durable readback. Do not repeat a completed provider readback merely because stdout was truncated. "
            "The top-level target in every owner evidence JSON must exactly equal the canonical target in paid-remote-intent.json and "
            ...
            "scheduled wake must resume immediately from those checkpoints. Canonical example: a contract requires 300 sends, a seller "
            "message says 300 are complete, but the paired official provider ledger proves only 12; classify it actionable remote and "
            "continue from 12. A seller-authored total is never completion proof when official effect receipts contradict it. "
