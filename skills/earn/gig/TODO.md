...
|---|---|---|---|---|---|
| Coconala | **Operational path recovered; fresh-effect gate open.** Natural run `18d41dfcd14b36c8-46829` passed from release `fe529db72`, observed 80 officially unavailable postings, produced no effect/failure/pending, and delivered Telegram `75427` once. The former restriction is lifted; the next eligible proposal still needs official application readback and following replay-zero. | **Current continuous owner accepted.** PRs `#5010` and `#5011` add exact pre-effect thread-navigation retry and bounded inventory retry; PR `#5012` adds the shared observation-transient contract as main `ae4cff2a`. Target-only receipt `1d3c873655f62aa5e8957714` loaded immutable release `20260911T150100-ae4cff2a`. Natural run `18d42f0434e77828-21263` ended PASS/exit `0` with installed/event SHA match: `observed=174`, `actionable=15`, `readback=159`, `failed=0`, `pending=15`, `effect=0`. Four uncertain old intents remain safely fenced as temporary readback pending, eleven remain in durable backoff, and 159 replay with zero effect. | **Reference lifecycle accepted.** Five-room classification, failed zero, Ryu replay-zero, formal delivery off and durable external ownership for `18180857` remain the reference evidence. Current monitoring must still surface new buyer work naturally. | **Implementation/history exists; current conversion and fresh owner acceptance remain later in the fixed order.** | Retained Coconala customer revenue exists, but no new post-recovery acquisition-to-bank receipt chain is proved yet. |
| Lancers | **Continuous path accepted; fresh effects continue naturally.** Project `5600212` reconciled exactly once as proposal `27913800` with Telegram-once and replay-zero. Removed project `5599976` is now classified from the authenticated official detail as provider-terminal, removed from pending without resubmission, and installed main release `792538e9` ended consecutive natural PASS/exit `0` terminals at `2026-09-11T01:52:01Z` and `01:54:51Z`. | **Scheduling action accepted.** The `pyrite` request produced one booking, Calendar readback, buyer-visible completion and following replay-zero through the shared Reply lifecycle. Continue observing other threads. | **Empty official contract inventory only.** Runtime monitoring passes, but no real funded delivery/acceptance/payment chain exists. | **Code/history exists; current attributable order and payment are unproved.** The older state/readback branch must be reviewed against current main rather than copied. | No verified contract, payout or new bank receipt. The immediate cause is upstream conversion/contract absence, not an unbuilt second Paid kernel. |
| CrowdWorks | **Operational and producing official applications.** Full release `fe529db72` ended natural PASS `18d41d9e94331ae8-1832`, inspecting 38 postings with 33 off-topic and two not workable, effect zero and no ledger change. The later natural owner run at `2026-09-11T06:35:57Z` submitted project `13444288` and officially read back proposal `305445161` with `submitted=true`, `application_verified=true` and `effect_delta=1`. Earlier proposal receipts remain real; Gmail is not the acceptance authority. The authenticated `/e/proposals` list plus exact proposal detail is official proof. The following identity-specific replay-zero remains to be captured. | **Contract transition accepted.** Contracts `63570481` and `63568785` have official readback and Telegram-once. The earlier Google Form confirmation message did not prove form completion; after contract `63570481` became funded, ownership correctly moved to Paid. Reply must not keep asking the client to confirm a seller-owned action. | **Funded work is active but not yet accepted.** Contract `63570481` is officially funded. Its bound external-action receipt proves the Google Form submission `confirmed` at `2026-09-10T22:37:40Z`; the loop must never submit that form again. Contracts `63568785` and `63583795` are officially waiting for escrow. PR `#5013`, merged as `5372d2a9`, serializes provider browser work. Natural run `18d4306a8ef8b0b8-47158` and the following installed run both ended with only funded item `63570481` failed as `TimeoutError`, while the two unfunded items remained durable pending. PR `#5015`, merged as `953c12d2`, exposed the exact failure: the official page contains duplicate milestone forms and `.last` selected the hidden textarea. PR `#5016` proved that form-container visibility is not the right predicate because the container has no display box. PR `#5018`, merged as `a53e5fdc`, now selects the single exact milestone form whose message textarea is visible, independent of DOM order, while keeping the Google Form receipt fence and failing closed on zero/multiple candidates. Dependency-complete immutable release `20260911T163008-a53e5fdc` is installed only for Paid with receipt `448decaf860772ccbc047426`. Its first natural run ended before any item or effect at `provider_inventory` with a generic `RuntimeError`; therefore the visible-textarea production path is still untested and completion remains open. The next natural terminal must traverse inventory, send the CrowdWorks milestone completion, prove official readback, and the following wake must prove replay-zero. | **No official seller-storefront capability is currently proved.** Do not invent this lane unless provider inspection shows one. | One funded contract is immediately monetizable, but accepted delivery/payment/bank receipt is not yet proved. |
| Mercor | **Accepted with material-fit boundary and replay safety.** PR `#5004` defines shared high/medium/low ranking; PR `#5005`, merged as `fe149d2d`, makes only material required contradictions non-submittable; PR `#5007`, merged as `8e0a1a4d`, repairs only the exact pre-tool code-mode-host transient without risking duplicate effects. Natural run `mercor-20260911-140708-63079` inspected 15 official items: every material-low candidate was classified `no_reasonable_shot` or `low_fit_person_bound_skipped`, none was submitted, qualified high/medium candidates continued, and no application or human-gate ledger identity was duplicated. A qualified job whose application requires a person-bound interview becomes one durable Telegram handoff with the exact job/link/action; it never blocks discovery or applications to the remaining jobs. The installed owner ended naturally at `2026-09-11T05:18:01Z`, release/event SHA matched `8e0a1a4d`, exit was `0`, and Telegram terminal report `75960` was delivered once. Earlier Python submission and following replay-zero remain accepted evidence. | **Accepted.** Consecutive natural passes cover the authenticated in-platform/application event inventory; email is not treated as a platform reply. | **Only official empty-contract monitoring is proved.** Real task preparation, one durable human handoff for physically person-bound audio/video work, loop resumption after the returned artifact, official submission, acceptance and payout remain unaccepted. | **Not applicable on current official surface.** | Applications exist, but no accepted paid work or payout is proved. |
...
Mercor PR `#5004` is merged as `92cf6b50`, with `115` focused tests plus the shared runtime and
registry suites passing; target-only install receipt `a45bc71de1b099610e06bc5a` is loaded. The
first natural run `18d423d23f7c5250-80399` passed and submitted the medium-fit Python listing with
official readback and Telegram application receipt `75661`. Run `18d4261d3ee9d360-30246` proves
replay-zero for that identity, but also exposed the next defect: it submitted low-fit Go listing
`list_AAABoDfKk7rrUXpQF2VF7Z3e` despite recording that production Go and required seniority are
unverified. PR `#5005` is merged to main as `fe149d2d`; `115` focused tests plus `389` runtime and
...
`4899367d369548f040317d56`. Its first natural owner run `18d4294202e555b0-4482` had exit zero but
business result `blocked`, inspections zero and applications zero because the Codex tool host timed
out before any tool action; Telegram `75840` reported the blocker. Shared runner PR `#5007`, merged
as `8e0a1a4d`, now treats that exact pre-tool condition as transient and uses the configured fallback,
while any started tool item remains effect-uncertain and forbids fallback. Immutable release
`20260911T133325-8e0a1a4d` is loaded only for Mercor Apply with install receipt
`0648d8f832b59c9b07cbac2b`. Its natural run `mercor-20260911-140708-63079` inspected 15 official
items, skipped all material-low candidates without submission, continued qualified high/medium
human-gate handling, added no duplicate application or gate identity, delivered Telegram `75960`
once, and ended naturally with matching release/event SHA and exit `0`. Mercor's skill SSOT states
the same missing-evidence versus material-contradiction boundary via PR `#5006`, merged as
`d4e0e2ba`, so a future adapter repair cannot lawfully restore the former submit-low behavior.
Coconala Reply is not currently accepted: installed natural run `18d42c88c630aa30-75724` ended
`exit 1`. Read-only official inspection proves an authenticated five-page inbox, while four thread
reads fail before effect with `authenticated tab did not finish navigation`. Commit `f877dcb43d` on
`fix/coconala-reply-pagination-terminal-20260911` retries only that exact pre-effect read once using a
fresh hidden tab; `135` focused Reply tests and the shared kernel/registry suites pass. It remains
unmerged and uninstalled pending fresh review and production acceptance.
...
absent and no proposal was resubmitted. Mercor Apply material-fit acceptance is also closed by the
natural installed-owner evidence recorded in the authoritative snapshot. The active engineering
atom is now CrowdWorks Paid funded-work acceptance. This preserves the fixed
...
- **Lancers:** the earlier dead-CDP diagnosis is stale. Official CDP `9227` responds and installed
  ...
  from pending without resubmission and ended consecutive natural PASS/exit `0` terminals at
  `2026-09-11T01:52:01Z` and `01:54:51Z`; the second wake is the replay-zero proof.
