...
- **Chii `18180857`:** the official ledger proves 12 valid TikTok DM/Sheet pairs; the seller-authored
  ...
  because of the nested target-ledger bug. The repaired natural wake proves the official `@anicca.jp`
  identity is authenticated, so login is not the current blocker. The measured throughput defect is the
  execution shape: a generic Paid agent receives a very large project context and prompt, rediscovers old
  artifacts, audits hashes and invents browser steps before each small effect. The five-minute schedule is
  only a wake cadence; it is not a 300-send batch executor. Replace that repeated exploration with one
  shared, checkpointed campaign tool: the model selects and qualifies candidates and composes truthful
  recipient-specific text, while deterministic code owns the candidate cursor, TikTok send/readback,
  paired Sheet append/readback, deduplication, rate-limit observation and resume. One owner run continues
  bounded batches without rereading the full history until 300 official pairs exist or TikTok returns an
  official cooldown/challenge. Completion remains 300 paired receipts, reply/contract tracking and no
  duplicate send; a process timeout or next wake is never treated as buyer work.
...
0. **Existing funded liabilities first.** Finish every already-purchased Coconala/CrowdWorks obligation
   ...
   replay-zero. Coconala `18211838` (逃げ因子) is complete by Dais's official confirmation and is excluded
   from every active and future TODO; never recheck, withdraw, replace or resend it. Continue only with the
   remaining already-funded Coconala/CrowdWorks liabilities.
   Coconala Ryu `18211957` is **not delivered**. Fresh saved official state is `取引中`,
   `formal_delivery_confirmed=false`, the formal-delivery checkbox remains enabled/unchecked, and
   `buyer_feedback_pending_artifact=true`. The buyer's existing FTP history and latest budget-scoped
   management-screen request belong to this owner's cumulative context and must not be rediscovered from
   another client. As soon as the safe complete release is current, this owner independently resumes,
   verifies the requested revision on the production site, sends the buyer-visible explanation and
   artifact once with `正式な納品` enabled, reads back the exact room's submitted state, then proves
   replay-zero. It does not wait for any other client. Manledge/Clover `18169985` is closed
   and excluded.
...
3. [x] `COCONALA-PAID-3` Restore continuous Paid health after the reference pass regressed.
   ...
   - [x] `COCONALA-PAID-3A` Finish Ryu's current revision cycle. Paid consumed the cumulative survey,
     ...
     A later campaign-banner request reopened this room. Paid completed that cycle through the same
     shared path: owner and fresh verifier matched requirements `13e15235...830859b`, the connector
     sent one normal message with effect/readback `1/1`, exact live-DOM message SHA
     `ff03c6e3...e595d`, and formal delivery off. Release `5824f60a...` is installed; the next natural
     aggregate initially appeared to own duplicate-zero confirmation. Fresh official readback instead
     exposes a later buyer request: explain every budget-included change and provide the updated
     management screen. The room remains `buyer_feedback_pending_artifact=true`. The shared replay
...
Coconala therefore is **operational but not platform-complete**. The active cursor is `COC-05`;
...
action, but does not serialize already-installed providers or independent clients: they continue to run
  concurrently. The implementation cursor is `COC-05`. Completed Ryu, Chii-excluded, 逃げ因子 and
...
- [x] `COC-02` Rebase the feature branch on current public `main`, rerun the affected suite and obtain
  a fresh read-only `ship` review for the final diff. Branch HEAD `93ee1bd6c2` is based directly on
  public main `1c9cb26bb` (`behind=0`). The final focused suite passes 40 tests and the complete Gig
  suite passes 1,270 tests. The repair removes the import-callable submit bypass, forbids same-title
  success attribution, binds confirmation to the exact listing ULID and official applied-page title,
...
- [x] `COC-04` Build one main-derived immutable release; install it on Coconala Apply only through the
  repository release path; verify installed SHA and owner configuration. Canonical
  `bin/cut-loop-release.sh origin/main` produced `20260914T095539-03fd32af`; targeted `lm-loop apply`
  changed only `hf-gig-apply-direct` with install event `13f2796f922dc7111fc15965`. `lm-loop status`
