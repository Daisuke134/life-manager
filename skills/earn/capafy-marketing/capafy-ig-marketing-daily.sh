...
NPX_BIN="${CAPAFY_NPX:-${LIFE_MANAGER_NPX:-$(command -v npx 2>/dev/null)}}"
...
"$PYTHON_BIN" "$LIFE_MANAGER_REPO/skills/earn/capafy-marketing/scripts/ig_metrics.py" >>"$LOG" 2>&1 || echo "ig_metrics failed (non-fatal)" >>"$LOG"
"$PYTHON_BIN" "$LIFE_MANAGER_REPO/skills/earn/capafy-marketing/scripts/pull_attribution.py" >>"$LOG" 2>&1 || echo "pull_attribution failed (non-fatal)" >>"$LOG"
...
if "$PYTHON_BIN" "$LIFE_MANAGER_REPO/skills/earn/capafy-marketing/scripts/build_landing.py" >>"$LOG" 2>&1; then
  LANDING_AFTER="$(landing_fingerprint)"
  if [ "$LANDING_BEFORE" = "$LANDING_AFTER" ]; then
    echo "landing unchanged; deploy skipped" >>"$LOG"
  elif [ -z "$NPX_BIN" ]; then
    echo "npx is required to deploy the Capafy landing page" >>"$LOG"
    RC=2
  elif ! ( cd "$LIFE_MANAGER_REPO/skills/earn/capafy-marketing" && "$NPX_BIN" --yes netlify-cli@27.1.2 deploy --prod --dir "$LANDING_SITE" --site "$LANDING_SITE_ID" ) >>"$LOG" 2>&1; then
...
COMMERCIAL_OK="no"
if "$PYTHON_BIN" - "$COMMERCIAL_MARKER" "$IG_HANDLE" <<'PY'
import json, os, sys
try:
...
if [ "$PROVISION_NEEDED" = "no" ]; then
  SELECTED_JSON="$("$PYTHON_BIN" "$SCRIPT_DIR/scripts/select_listing.py")" || {
    echo "evidence-ready listing selection failed: $SELECTED_JSON" >>"$LOG"
    exit 1
  }
  SELECTED_AGENT_ID="$(printf '%s' "$SELECTED_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin).get("agent_id") or "")')"
  [ -n "$SELECTED_AGENT_ID" ] || exit 1
  CAMPAIGN_URL="${LANDING_URL%/}/go/${SELECTED_AGENT_ID}"
  [ -f "$IG_LEDGER" ] && PRE_IG_ROWS="$(wc -l < "$IG_LEDGER" | tr -d ' ')"
  APPROVAL_JSON="$("$PYTHON_BIN" "$SCRIPT_DIR/scripts/creative_approval.py" \
  ...
  CREATIVE_APPROVAL_STATUS="$(printf '%s' "$APPROVAL_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["status"])')"
  ...
  if [ "$CREATIVE_APPROVAL_STATUS" = "approved" ]; then
    APPROVED_ARTIFACT_PATH="$(printf '%s' "$APPROVAL_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["artifact_path"])')"
    APPROVED_ARTIFACT_SHA256="$(printf '%s' "$APPROVAL_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["artifact_sha256"])')"
  fi
fi
...
if [ "$RC" -eq 0 ] && [ "$PROVISION_NEEDED" = "no" ] && [ "$MODE_FLAG" = "--live" ]; then
  VERIFIED_POST="$("$PYTHON_BIN" - "$IG_LEDGER" "$PRE_IG_ROWS" "$SELECTED_AGENT_ID" <<'PY'
...
PY
  ...
  if [ -z "$VERIFIED_POST" ]; then
    ...
    "$PYTHON_BIN" "$SCRIPT_DIR/scripts/select_listing.py" --commit-agent-id "$SELECTED_AGENT_ID" >>"$LOG" 2>&1 || RC=4
  fi
fi
